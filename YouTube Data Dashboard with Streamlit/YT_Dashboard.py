...
@st.cache_data  # Like a coffee machine that remembers your order instead of making it from scratch every time.
def load_data():
...
#create data frame from the funcation
...
#just numeric coulmns 
numeric_col = np.array((df_agg_diff.dtypes == 'float64') | (df_agg_diff.dtypes == 'int64'))
df_agg_diff.iloc[:,numeric_col] = (df_agg_diff.iloc[:,numeric_col] - median_agg).div(median_agg)
...
# streamlit app 
add_sidebar = st.sidebar.selectbox('Aggregated or Individual video', ('Aggregated Metrics', 'Individual video Analysis'))

if add_sidebar == 'Aggregated Metrics': 
    st.write('Ken Jee Youtube Aggregated Data')
    #[[ ]] - for MULTIPLE columns  reuturn 2D data frame 
    df_agg_metrics = df_agg[['Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed',
                             'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']]
    video_dates = df_agg['Video publish time']
    metric_data_6mo = video_dates.max() - pd.DateOffset(months = 6)
    metirc_data_12mo = video_dates.max() - pd.DateOffset(months = 12)
    metirc_median_6mo = df_agg_metrics[video_dates>= metric_data_6mo].median()
    metirc_median_12mo = df_agg_metrics[video_dates>= metirc_data_12mo].median()
    
    col1, col2, col3, col4, col5 = st.columns(5)
    cols = [col1, col2, col3, col4, col5]    
    
    count = 0
    for i in metirc_median_6mo.index:
        with cols[count]:
            delta = (metirc_median_6mo[i] - metirc_median_12mo[i])/metirc_median_12mo[i]
            st.metric(label= i ,value=round(metirc_median_6mo[i],1), delta="{:.2%}".format(delta))
            count +=1
            if count >=5:
                count =0
